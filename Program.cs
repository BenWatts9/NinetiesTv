...
namespace NinetiesTV
{
    class Program
    {
        ...
        **************************************************************************************************/
        ...
        {
            List<string> showList = shows.Select(s => s.Name).ToList();
            return showList.OrderBy(s => s).ToList();
        }
        ...
        {
            return shows.Where(p => p.EndYear < 2000 && p.StartYear > 1989).ToList();
        }

        // 8. Return the top three highest rated shows.
        static List<Show> TopThreeByRating(List<Show> shows)
        {
            List<Show> showList = shows.OrderByDescending(s => s.ImdbRating).ToList();
            return showList.Take(3).ToList();
        }
        ...
        {
            List<Show> showList = shows.Where(s => s.EpisodeCount < 100).ToList();
            return showList.Select(s => s.Name).ToList();
        }
        ...
        {
            List<string> showList = shows.Select(s => s.Name).ToList();
            string lastShow = showList[showList.Count - 1];
            showList.RemoveAt(showList.Count - 1);

            return String.Join(", ", showList) + ", and " + lastShow; 
